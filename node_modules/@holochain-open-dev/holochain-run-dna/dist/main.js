...
var nodePlatformShim = {
    fs: {
        ...
        writeFile: fs__default['default'].writeFile
    },
    format: util__default['default'].format,
    resolve: path__default['default'].resolve,
    exists: (file) => {
        try {
            return fs__default['default'].statSync(file).isFile();
        }
        catch (err) {
            return false;
        }
...
const parser = new YargsParser({
    ...
    format: util__default['default'].format,
    normalize: path__default['default'].normalize,
    resolve: path__default['default'].resolve,
    require: (path) => {
...
const stringWidth$2 = require('string-width');
const stripAnsi$2 = require('strip-ansi');
...
const wrapWord = (rows, word, columns) => {
	...
	let visible = stringWidth$2(stripAnsi$2(rows[rows.length - 1]));
...
var wrapAnsi$1 = /*#__PURE__*/Object.freeze({
  __proto__: null
});

var require$$7 = /*@__PURE__*/getAugmentedNamespace(stringWidth$1);

var stripAnsi$3 = /*@__PURE__*/getAugmentedNamespace(stripAnsi$1);

var wrap = /*@__PURE__*/getAugmentedNamespace(wrapAnsi$1);
...
function ui(opts) {
    return cliui(opts, {
        stringWidth: require$$7,
        stripAnsi: stripAnsi$3,
        wrap
    });
}
...
var getCallerFile = /*#__PURE__*/Object.freeze({
  __proto__: null
});
...
var requireDirectory$1 = /*#__PURE__*/Object.freeze({
  __proto__: null
});

var require$$4 = /*@__PURE__*/getAugmentedNamespace(sync);

var require$$5 = /*@__PURE__*/getAugmentedNamespace(getCallerFile);

var require$$6 = /*@__PURE__*/getAugmentedNamespace(requireDirectory$1);
...
function Yargs(processArgs = [], cwd = shim$1$1.process.cwd(), parentRequire) {
    const self = {};
    let command$1;
    ...
    let default$0;
    if (/\b(node|iojs|electron)(\.exe)?$/.test(shim$1$1.process.argv()[0])) {
        default$0 = shim$1$1.process.argv().slice(1, 2);
    }
    else {
        default$0 = shim$1$1.process.argv().slice(0, 1);
    }
    self.$0 = default$0
        .map(x => {
        ...
        .join(' ')
        .trim();
    if (shim$1$1.getEnv('_') && shim$1$1.getProcessArgvBin() === shim$1$1.getEnv('_')) {
        self.$0 = shim$1$1
            .getEnv('_')
            .replace(`${shim$1$1.path.dirname(shim$1$1.process.execPath())}/`, '');
    }
    ...
    self.resetOptions = self.reset = function resetOptions(aliases = {}) {
        ...
        options = tmpOptions;
        usage$1 = usage$1 ? usage$1.reset(localLookup) : usage(self, y18n, shim$1$1);
        validation$1 = validation$1
            ? validation$1.reset(localLookup)
            : validation(self, usage$1, y18n, shim$1$1);
        command$1 = command$1
            ? command$1.reset()
            : command(self, usage$1, validation$1, globalMiddleware, shim$1$1);
        if (!completion$1)
            completion$1 = completion(self, usage$1, command$1, shim$1$1);
    ...
    function unfreeze() {
        const frozen = frozens.pop();
        assertNotStrictEqual(frozen, undefined, shim$1$1);
        let configObjects;
        ({
            options,
            configObjects,
    ...
    self.config = function config(key = 'config', msg, parseFn) {
        ...
        if (typeof key === 'object' && !Array.isArray(key)) {
            key = applyExtends(key, cwd, self.getParserConfiguration()['deep-merge-config'] || false, shim$1$1);
    ...
    self.commandDir = function (dir, opts) {
        ...
        const req = parentRequire || shim$1$1.require;
        command$1.addDirectory(dir, self.getContext(), req, shim$1$1.getCallerFile(), opts);
        return self;
    };
    self.demand = self.required = self.require = function demand(keys, max, msg) {
        if (Array.isArray(max)) {
            max.forEach(key => {
                assertNotStrictEqual(msg, true, shim$1$1);
    ...
    self.usage = function (msg, description, builder, handler) {
        ...
        if (description !== undefined) {
            assertNotStrictEqual(msg, null, shim$1$1);
            if ((msg || '').match(/^\$0( |$)/)) {
    ...
    self.pkgConf = function pkgConf(key, rootPath) {
        ...
        if (obj[key] && typeof obj[key] === 'object') {
            conf = applyExtends(obj[key], rootPath || cwd, self.getParserConfiguration()['deep-merge-config'] || false, shim$1$1);
    ...
    function pkgUp(rootPath) {
        ...
        let obj = {};
        try {
            let startDir = rootPath || shim$1$1.mainFilename;
            if (!rootPath && shim$1$1.path.extname(startDir)) {
                startDir = shim$1$1.path.dirname(startDir);
            }
            const pkgJsonPath = shim$1$1.findUp(startDir, (dir, names) => {
            ...
            assertNotStrictEqual(pkgJsonPath, undefined, shim$1$1);
            obj = JSON.parse(shim$1$1.readFileSync(pkgJsonPath, 'utf8'));
        }
        catch (_noop) { }
        pkgs[npath] = obj || {};
        return pkgs[npath];
    ...
    self.terminalWidth = () => {
        argsert([], 0);
        return shim$1$1.process.stdColumns;
    };
    ...
    self._parseArgs = function parseArgs(args, shortCircuit, _calledFromCommand, commandIndex) {
        ...
        const parsed = shim$1$1.Parser.detailed(args, Object.assign({}, options, {
    ...
    self._parsePositionalNumbers = function (argv) {
        ...
        for (let i = 0, arg; (arg = args[i]) !== undefined; i++) {
            if (shim$1$1.Parser.looksLikeNumber(arg) &&
    ...
    function guessLocale() {
        if (!detectLocale)
            return;
        const locale = shim$1$1.getEnv('LC_ALL') ||
            shim$1$1.getEnv('LC_MESSAGES') ||
            shim$1$1.getEnv('LANG') ||
            shim$1$1.getEnv('LANGUAGE') ||
            'en_US';
...
const rebase = (base, dir) => shim$1$1.path.relative(base, dir);
function isYargsInstance(y) {
    return !!y && typeof y._parseArgs === 'function';
}

var _a, _b;
const { readFileSync } = fs__default['default'];
const { inspect } = util__default['default'];
const { resolve: resolve$1 } = path__default['default'];


var cjsPlatformShim = {
    assert: {
        notStrictEqual: assert__default['default'].notStrictEqual,
        strictEqual: assert__default['default'].strictEqual,
    },
    cliui: build$2,
    findUp: require$$4,
    getEnv: (key) => {
    ...
    requireDirectory: require$$6,
    stringWidth: require$$7,
    y18n: build({
        directory: resolve$1(__dirname, '../locales'),
        updateFiles: false,
    }),
};
...
if (process && process.version) {
    ...
    if (major < minNodeVersion$1) {
        throw Error(`yargs supports a minimum Node.js version of ${minNodeVersion$1}. Read our version support policy: https://github.com/yargs/yargs#supported-nodejs-versions`);
    }
}
const Parser$1 = build$1;
...
const {Yargs: Yargs$2, processArgv: processArgv$1} = build$3;

Argv(processArgv$1.hideBin(process.argv));

var yargs = Argv;

function Argv(processArgs, cwd) {
  const argv = Yargs$2(processArgs, cwd, commonjsRequire);
  singletonify(argv);
  return argv;
}
...
const {
  applyExtends: applyExtends$1,
  cjsPlatformShim: cjsPlatformShim$1,
  Parser,
  processArgv: processArgv$2,
} = build$3;

var helpers = {
  applyExtends: (config, cwd, mergeExtends) => {
    return applyExtends$1(config, cwd, mergeExtends, cjsPlatformShim$1);
  },
  hideBin: processArgv$2.hideBin,
  Parser,
};

function getDnaPath(provisionalPath) {
  if (fs__default['default'].existsSync(provisionalPath)) {
    return provisionalPath;
  } else if (fs__default['default'].existsSync(__dirname + provisionalPath)) {
...
function getAppToInstall() {
  const yarg = yargs(helpers.hideBin(process.argv)).help()
        .option("port", {
          alias: "p",
          type: "integer",
          default: 8888,
...
var rimraf$1 = /*#__PURE__*/Object.freeze({
  __proto__: null
});

var rimraf$2 = /*@__PURE__*/getAugmentedNamespace(rimraf$1);
...
function _prepareTmpDirRemoveCallback(name, opts, sync) {
  const removeFunction = opts.unsafeCleanup ? rimraf$2 : fs__default['default'].rmdir.bind(fs__default['default']);
...
function createConfigFile(adminPort, dirName, proxyUrl) {
  ...
  if (fs__default['default'].existsSync(configFileName)) {
    try {
      const doc = yaml.safeLoad(fs__default['default'].readFileSync(configFileName, 'utf8'));
...
`;
  ...
  fs__default['default'].writeFileSync(configFileName, configFileContents);
...
async function execHolochain(adminPort, runPath, proxyUrl) {
  ...
  child_process__default['default'].spawn("lair-keystore", [], {
    stdio: "inherit",
    env: process.env,
  });
  ...
  child_process__default['default'].spawn("holochain", ["-c", configFilePath], {
    stdio: "inherit",
    env: {
      ...process.env,
...
var node = require$$0__default['default'];
...
const TEXT_ENCODING_AVAILABLE = typeof process !== "undefined" &&
...
class Decoder {
    ...
    decodeUtf8String(byteLength, headerOffset) {
        ...
        else if (byteLength > utf8.TEXT_DECODER_THRESHOLD) {
...
async function decodeAsync(streamLike, options = decode_1.defaultDecodeOptions) {
    ...
    const decoder = new Decoder_1.Decoder(options.extensionCodec, options.context, options.maxStrLength, options.maxBinLength, options.maxArrayLength, options.maxMapLength, options.maxExtLength);
    return decoder.decodeAsync(stream$1);
}
...
const msgpack = __importStar(dist);
...
async function installApp(adminPort, appPort, dnas, installedAppId) {
  const adminWebsocket = await lib.AdminWebsocket.connect(
  ...
  const appWebsocket = await lib.AppWebsocket.connect(`ws://localhost:${appPort}`);
...
async function attachAppInterface(adminPort, appPort, installedAppId) {
  console.log("\n Attaching... \n ");
  const adminWebsocket = await lib.AdminWebsocket.connect(
    `ws://localhost:${adminPort}`
  );
